#!/usr/bin/env python3
"""Repository-level v16 validation without Flutter/Dart execution.

Checks prior curriculum invariants plus the first timbre vertical slice,
day-weighted Tone Profile, v16 research recheck status, generated content
...
def validate_content_manifest() -> dict[str, object]:
    rel = "app/assets/curriculum/content_manifest_v16.json"
    path = ROOT / rel
    if not path.exists():
        fail(f"Missing content manifest: {rel}")
    manifest = json.loads(path.read_text(encoding="utf-8"))
    if manifest.get("schema") != "vocal-athlete/curriculum-content-manifest@2":
        fail("Unexpected v16 content manifest schema")
    if manifest.get("version") != "v16" or manifest.get("algorithm") != "sha256":
        fail("v16 content manifest version/algorithm mismatch")
    files = manifest.get("files")
    if not isinstance(files, dict) or len(files) != 31:
        fail(f"v16 content manifest must track 31 files, got {len(files or {})}")
    kinds: dict[str, int] = {}
    ...
    if kinds != expected:
        fail(f"Unexpected v16 manifest kind counts: {kinds}")
    proc = subprocess.run(
        [
            sys.executable,
...
def validate_timbre_code() -> dict[str, object]:
    ...
    if missing:
        fail(f"Missing v16 timbre implementation files: {missing}")
    ...
    for token in (
        ...
        if token not in profile:
            fail(f"v16 day-weighted Tone Profile marker missing: {token}")
    ...
    for token in (
        ...
        if token not in screen:
            fail(f"v16 Tone Profile screen marker missing: {token}")
...
def validate_research_bundle() -> dict[str, object]:
    ...
    if missing:
        fail(f"Missing v16 research files: {missing}")
    ...
    if register.get("version") != "v16" or not isinstance(sources, list):
    ...
    if statuses["spot_checked_v15"] != 8 or statuses["spot_checked_v16"] != 5:
        fail(f"Unexpected v15/v16 source check counts: {dict(statuses)}")
    if statuses["imported_pending_full_recheck"] != 26:
        fail(f"Expected 26 pending sources, got {statuses['imported_pending_full_recheck']}")
    r27 = by_id["R27"]["title"]
...
def validate_content_and_docs() -> dict[str, object]:
    required = [
        ...
        "docs/FEEDBACK-UPDATE-2026-06-21-v16.md",
        "docs/UPDATE-VALIDATION-2026-06-21-v16.md",
        "docs/NEXT-VERSION-DIRECTION-v17.md",
        "docs/PATCH-MANIFEST-v16.json",
        "docs/changed-files-v16.txt",
        "app/assets/curriculum/content_manifest_v16.json",
        "app/assets/curriculum/beginner_timbre_slice_v16.json",
        "app/assets/training/timbre_v16/rights.json",
        "tools/flutter_environment_v16.txt",
    ]
    missing = [rel for rel in required if not (ROOT / rel).exists()]
    if missing:
        fail(f"Missing v16 docs/files: {missing}")
    ...
    pubspec = (ROOT / "app/pubspec.yaml").read_text(encoding="utf-8")
    if "version: 1.16.0+16" not in pubspec:
        fail("pubspec version is not 1.16.0+16")
    versioning = (ROOT / "docs/VERSIONING.md").read_text(encoding="utf-8")
    if "Current project version: **v16**" not in versioning:
        fail("VERSIONING.md does not identify v16")

    status = json.loads(
        (ROOT / "docs/verification/verification-status.json").read_text(encoding="utf-8")
    )
    if status.get("version") != "v16" or status.get("appVersion") != "1.16.0+16":
        fail("verification-status v16 version mismatch")
    if "timbreVerticalSliceV16" not in status.get("items", {}):
        fail("verification-status missing timbreVerticalSliceV16 item")

    patch = json.loads((ROOT / "docs/PATCH-MANIFEST-v16.json").read_text(encoding="utf-8"))
    if patch.get("version") != "v16" or patch.get("target") != "v16 / app 1.16.0+16":
        fail("PATCH-MANIFEST-v16 version/target mismatch")
    ...
    for token in (
        ...
        if token not in workflow + run_script:
            fail(f"v16 validation pipeline marker missing: {token}")

    blueprint = (ROOT / "app/lib/curriculum/lesson_blueprint.dart").read_text(encoding="utf-8")
    if "content_manifest_v16.json" not in blueprint:
        fail("Runtime blueprint repository does not use v16 manifest")

    return {"requiredDocs": len(required), "version": "v16"}

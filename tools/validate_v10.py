#!/usr/bin/env python3
"""Repository-level v10 smoke validation that does not require Flutter/Dart.

This validates document/data syntax, card/path references, and the canonical
v10 curriculum and first vertical-slice invariants. It does not prove Flutter compilation, device audio
...
def validate_docs_and_version() -> dict[str, object]:
    required = [
        ...
        "docs/research/v10/IMPLEMENTATION-ANCHORS.md",
        "docs/adr/0028-first-learning-vertical-slice-v10.md",
        "docs/FEEDBACK-UPDATE-2026-06-20-v10.md",
        "docs/UPDATE-VALIDATION-2026-06-20-v10.md",
        "docs/PATCH-MANIFEST-v10.json",
    ]
    missing = [p for p in required if not (ROOT / p).exists()]
    if missing:
        fail(f"Missing v10 documents: {missing}")

    pubspec = (ROOT / "app/pubspec.yaml").read_text(encoding="utf-8")
    if "version: 1.10.0+10" not in pubspec:
        fail("pubspec version is not 1.10.0+10")
    verification = json.loads((ROOT / "docs/verification/verification-status.json").read_text(encoding="utf-8"))
    if verification.get("version") != "v10" or verification.get("appVersion") != "1.10.0+10":
        fail("verification version markers do not match v10")
    versioning = (ROOT / "docs/VERSIONING.md").read_text(encoding="utf-8")
    if "Current project version: **v10**" not in versioning:
        fail("VERSIONING.md does not identify v10")
    patch_manifest = json.loads((ROOT / "docs/PATCH-MANIFEST-v10.json").read_text(encoding="utf-8"))
    if patch_manifest.get("version") != "v10" or patch_manifest.get("target") != "v10 / app 1.10.0+10":
        fail("PATCH-MANIFEST-v10.json markers mismatch")
    return {"requiredDocs": len(required), "version": "v10", "appVersion": "1.10.0+10"}

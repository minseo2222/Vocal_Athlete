#!/usr/bin/env python3
"""Repository-level v12 smoke validation that does not require Flutter/Dart.

This validates document/data syntax, card/path references, and the canonical
v12 curriculum and first vertical-slice invariants. It does not prove Flutter compilation, device audio
...
def validate_v11_legacy_features() -> dict[str, object]:
    required = [
        ...
        "docs/adr/0029-learning-evidence-audio-interlock-v12.md",
    ]
    missing = [rel for rel in required if not (ROOT / rel).exists()]
    if missing:
        fail(f"Missing v12 feature files: {missing}")
    ...
    for marker in (
        ...
        if marker not in lesson_screen:
            fail(f"LessonScreen missing v12 marker: {marker}")
...
def validate_docs_and_version() -> dict[str, object]:
    required = [
        ...
        "docs/adr/0030-delayed-review-queue-v12.md",
        "docs/FEEDBACK-UPDATE-2026-06-20-v12.md",
        "docs/UPDATE-VALIDATION-2026-06-20-v12.md",
        "docs/PATCH-MANIFEST-v12.json",
    ]
    missing = [p for p in required if not (ROOT / p).exists()]
    if missing:
        fail(f"Missing v12 documents: {missing}")

    pubspec = (ROOT / "app/pubspec.yaml").read_text(encoding="utf-8")
    if "version: 1.12.0+12" not in pubspec:
        fail("pubspec version is not 1.12.0+12")
    verification = json.loads((ROOT / "docs/verification/verification-status.json").read_text(encoding="utf-8"))
    if verification.get("version") != "v12" or verification.get("appVersion") != "1.12.0+12":
        fail("verification version markers do not match v12")
    if "learningEvidence" not in verification.get("items", {}) or "audioSessionIntegrity" not in verification.get("items", {}):
        fail("verification-status missing v12 evidence/audio items")
    versioning = (ROOT / "docs/VERSIONING.md").read_text(encoding="utf-8")
    if "Current project version: **v12**" not in versioning:
        fail("VERSIONING.md does not identify v12")
    patch_manifest = json.loads((ROOT / "docs/PATCH-MANIFEST-v12.json").read_text(encoding="utf-8"))
    if patch_manifest.get("version") != "v12" or patch_manifest.get("target") != "v12 / app 1.12.0+12":
        fail("PATCH-MANIFEST-v12.json markers mismatch")
    return {"requiredDocs": len(required), "version": "v12", "appVersion": "1.12.0+12"}
...
def main() -> int:
    report = {
        ...
        "v12Features": validate_v12_features(),
...
def validate_v12_features() -> dict[str, object]:
    required = [
        'app/lib/assessment/review_queue.dart',
        'app/lib/lesson/review_queue_screen.dart',
        'app/test/review_queue_test.dart',
        'app/test/review_queue_screen_test.dart',
    ...
    if missing:
        fail(f'Missing v12 files: {missing}')
    pubspec = (ROOT / 'app/pubspec.yaml').read_text(encoding='utf-8')
    if 'version: 1.12.0+12' not in pubspec:
        fail('pubspec version is not 1.12.0+12')
    evidence = (ROOT / 'app/lib/assessment/learning_evidence.dart').read_text(encoding='utf-8')
    for token in ('contentRevision', 'recordedTakeIds', 'bestTakeId'):
        if token not in evidence:
            fail(f'learning evidence missing {token}')
    queue = (ROOT / 'app/lib/assessment/review_queue.dart').read_text(encoding='utf-8')
    for token in ('ReviewQueueItem', 'SharedPreferencesReviewQueueRepository', 'ReviewQueueScheduler', 'dueEpochDay'):
        if token not in queue:
            fail(f'review queue missing {token}')
    screen = (ROOT / 'app/lib/lesson/review_queue_screen.dart').read_text(encoding='utf-8')
    for token in ('review-queue-screen', 'review-task-complete', 'contentRevision'):
        if token not in screen:
            fail(f'review queue screen missing {token}')
    main = (ROOT / 'app/lib/main.dart').read_text(encoding='utf-8')
    for token in ('_showReviewQueue', 'ReviewQueueScreen', 'reviewQueueRepository'):
        if token not in main:
            fail(f'main.dart missing {token}')
    settings = (ROOT / 'app/lib/lesson/settings_screen.dart').read_text(encoding='utf-8')
    if 'settings-review-queue' not in settings or '1.12.0' not in settings:
        fail('settings screen missing review queue entry or v12 version')
    verification = json.loads((ROOT / 'docs/verification/verification-status.json').read_text(encoding='utf-8'))
    if verification.get('version') != 'v12' or verification.get('appVersion') != '1.12.0+12':
        fail('verification-status is not v12')
    if 'delayedReviewQueue' not in verification.get('items', {}):
        fail('verification-status missing delayedReviewQueue item')
    return {'v12Files': len(required), 'version': '1.12.0+12'}

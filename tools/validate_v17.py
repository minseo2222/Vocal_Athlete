#!/usr/bin/env python3
"""Repository-level v17 validation without Flutter/Dart execution.

Checks prior curriculum invariants plus the first timbre vertical slice,
day-weighted Tone Profile, v17 research recheck status, generated content
...
def validate_content_manifest() -> dict[str, object]:
    rel = "app/assets/curriculum/content_manifest_v17.json"
    path = ROOT / rel
    if not path.exists():
        fail(f"Missing content manifest: {rel}")
    manifest = json.loads(path.read_text(encoding="utf-8"))
    if manifest.get("schema") != "vocal-athlete/curriculum-content-manifest@2":
        fail("Unexpected v17 content manifest schema")
    if manifest.get("version") != "v17" or manifest.get("algorithm") != "sha256":
        fail("v17 content manifest version/algorithm mismatch")
    files = manifest.get("files")
    if not isinstance(files, dict) or len(files) != 31:
        fail(f"v17 content manifest must track 31 files, got {len(files or {})}")
    kinds: dict[str, int] = {}
    ...
    if kinds != expected:
        fail(f"Unexpected v17 manifest kind counts: {kinds}")
    proc = subprocess.run(
        [
            sys.executable,
...
def validate_timbre_code() -> dict[str, object]:
    ...
    if missing:
        fail(f"Missing v17 timbre implementation files: {missing}")
    ...
    for token in (
        ...
        if token not in profile:
            fail(f"v17 day-weighted Tone Profile marker missing: {token}")
    ...
    for token in (
        ...
        if token not in curation:
            fail(f"v17 Tone Profile curation marker missing: {token}")

    recording = (ROOT / "app/lib/recording/recording_ab.dart").read_text(encoding="utf-8")
    for token in ("toneProfileExcluded", "toneTagEditedEpochMs", "toneTagEditMemo"):
        if token not in recording:
            fail(f"RecordingTake v17 curation metadata missing: {token}")
    ...
    for token in (
        ...
        if token not in screen:
            fail(f"v17 Tone Profile screen marker missing: {token}")
...
def validate_research_bundle() -> dict[str, object]:
    root = ROOT / "docs/research/v17"
    required = [
        "README.md",
        "TIMBRE-SOURCE-RECHECK.md",
        "TIMBRE-SOURCE-REGISTER.csv",
        "timbre-source-register.json",
    ]
    missing = [name for name in required if not (root / name).exists()]
    if missing:
        fail(f"Missing v17 research files: {missing}")
    ...
    if register.get("version") != "v17" or not isinstance(sources, list):
    ...
    if statuses["spot_checked_v15"] != 8 or statuses["spot_checked_v16"] != 5 or statuses["spot_checked_v17"] != 6:
        fail(f"Unexpected v15/v16/v17 source check counts: {dict(statuses)}")
    if statuses["imported_pending_full_recheck"] != 20:
        fail(f"Expected 20 pending sources, got {statuses['imported_pending_full_recheck']}")
    r27 = by_id["R27"]["title"]
...
def validate_content_and_docs() -> dict[str, object]:
    required = [
        ...
        "docs/FEEDBACK-UPDATE-2026-06-21-v17.md",
        "docs/UPDATE-VALIDATION-2026-06-21-v17.md",
        "docs/NEXT-VERSION-DIRECTION-v17.md",
        "docs/PATCH-MANIFEST-v17.json",
        "docs/changed-files-v17.txt",
        "app/assets/curriculum/content_manifest_v17.json",
        "app/assets/curriculum/beginner_timbre_slice_v16.json",
        "app/assets/training/timbre_v16/rights.json",
        "tools/flutter_environment_v17.txt",
        "docs/research/v17/README.md",
        "docs/research/v17/TIMBRE-SOURCE-RECHECK.md",
        "docs/research/v17/TIMBRE-SOURCE-REGISTER.csv",
        "docs/research/v17/timbre-source-register.json",
    ]
    missing = [rel for rel in required if not (ROOT / rel).exists()]
    if missing:
        fail(f"Missing v17 docs/files: {missing}")
    ...
    pubspec = (ROOT / "app/pubspec.yaml").read_text(encoding="utf-8")
    if "version: 1.17.0+17" not in pubspec:
        fail("pubspec version is not 1.17.0+17")
    versioning = (ROOT / "docs/VERSIONING.md").read_text(encoding="utf-8")
    if "Current project version: **v17**" not in versioning:
        fail("VERSIONING.md does not identify v17")

    status = json.loads(
        (ROOT / "docs/verification/verification-status.json").read_text(encoding="utf-8")
    )
    if status.get("version") != "v17" or status.get("appVersion") != "1.17.0+17":
        fail("verification-status v17 version mismatch")
    if "toneProfileCurationV17" not in status.get("items", {}):
        fail("verification-status missing toneProfileCurationV17 item")

    patch = json.loads((ROOT / "docs/PATCH-MANIFEST-v17.json").read_text(encoding="utf-8"))
    if patch.get("version") != "v17" or patch.get("target") != "v17 / app 1.17.0+17":
        fail("PATCH-MANIFEST-v17 version/target mismatch")
    ...
    for token in (
        ...
        if token not in workflow + run_script:
            fail(f"v17 validation pipeline marker missing: {token}")

    blueprint = (ROOT / "app/lib/curriculum/lesson_blueprint.dart").read_text(encoding="utf-8")
    if "content_manifest_v17.json" not in blueprint:
        fail("Runtime blueprint repository does not use v17 manifest")

    return {"requiredDocs": len(required), "version": "v17"}

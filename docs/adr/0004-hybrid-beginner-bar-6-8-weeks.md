> R4 supersession note: 이 ADR의 “초급 이후 장르 트랙에서 곡과 함께 연속” 표현은 현재 `초급 Foundation → Universal Vocal Core → Repertoire Application → Advanced Genre Lab` 경로로 대체된다. 초급 48일을 입문 토대로 본 결정은 유지한다.
